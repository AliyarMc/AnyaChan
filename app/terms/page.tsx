...
export default function TermsPage() {
  return (
    <div className="min-h-screen bg-[#0c0f16] text-slate-200 font-sans">
      {/* Background Decor */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div className="absolute bottom-[-10%] left-[-5%] w-[40%] h-[40%] bg-red-600/[0.03] blur-[120px] rounded-full" />
      </div>

      <nav className="fixed top-0 w-full z-50 border-b border-white/[0.03] bg-[#0c0f16]/80 backdrop-blur-3xl px-6 h-20 flex items-center justify-between">
        <Link href="/" className="flex items-center gap-4 group">
          ...
          <span className="text-xl font-bold text-white font-outfit uppercase tracking-tighter">{process.env.NEXT_PUBLIC_BRAND_NAME || "Anya"} Engine</span>
        </Link>
        <Link href="/">
      ...
      <main className="relative z-10 pt-40 pb-32 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="glass border-white/5 rounded-2xl p-10 md:p-16 space-y-12 bg-gradient-to-br from-white/[0.01] to-transparent">
            <section className="space-y-6">
              ...
              <p className="text-slate-400 leading-relaxed font-medium">
                By integrating the {process.env.NEXT_PUBLIC_BRAND_NAME || "Anya"} Engine into your Discord server, you agree to abide by these terms. The engine is provided &quot;as is,&quot; and while we strive for 100% uptime through our neural edge clusters, we are not liable for any data loss resulting from third-party API disruptions.
              </p>
            </section>
            ...
            <section className="space-y-6">
              ...
              <p className="text-slate-400 leading-relaxed font-medium">
                You may not use the {process.env.NEXT_PUBLIC_BRAND_NAME || "Anya"} Engine for any illicit activities, including but not limited to: automated harassment, token logging, or raid coordination. Violation of these constraints will result in immediate neural deauthorization and blacklisting from the global cluster network.
              </p>
            </section>
            ...
            <div className="pt-12 border-t border-white/5">
              <p className="text-[10px] font-black uppercase text-slate-600 tracking-[0.4em]">
                March 2026 // Distributed via {process.env.NEXT_PUBLIC_BRAND_NAME || "Anya"} Neural Cloud
              </p>
            </div>
          </div>
        </div>
